...
static uint32_t get_pulse(float setpoint) {
    return PWM_MIN_DUTY + (uint32_t)( (PWM_MIN_DUTY - PWM_MIN_DUTY) * setpoint);
}

static int output_pwm_set(const struct device *dev, const float outputs[4]) {
    const struct output_pwm_config *config = (const struct output_pwm_config*)dev->config;

    printk("here we go again %d\n", (int)(1000*outputs[0]));

    pwm_pin_set_usec(config->pwm_dev[0], 4, PWM_PERIOD, get_pulse(outputs[0]), 0);
    pwm_pin_set_usec(config->pwm_dev[0], 3, PWM_PERIOD, get_pulse(outputs[1]), 0);
    pwm_pin_set_usec(config->pwm_dev[0], 2, PWM_PERIOD, get_pulse(outputs[2]), 0);
    pwm_pin_set_usec(config->pwm_dev[1], 1, PWM_PERIOD, get_pulse(outputs[3]), 0);
